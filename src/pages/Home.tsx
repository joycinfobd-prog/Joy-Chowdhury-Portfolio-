...
function Stat({ value, suffix, label, color }: StatItem) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <span className={`font-display text-[1.7rem] font-extrabold tracking-tight sm:text-4xl ${color}`}>
        <span ref={ref}>0</span>
        {suffix}
      </span>
      <span className="text-[10.5px] font-semibold uppercase leading-tight tracking-[0.1em] text-gray-500 sm:text-[11px] sm:tracking-[0.12em]">
        {label}
      </span>
    </div>
  );
  ...
  return (
    <div ref={root}>
      ...
      <section className="relative overflow-hidden pt-32 sm:pt-36 lg:pt-40">
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-14 px-6 lg:grid-cols-[1.12fr_0.88fr] lg:gap-10">
          {/* ------------------------------ copy ------------------------------ */}
          <div>
            ...
            <h1 className="mt-5 font-display font-extrabold tracking-[-0.04em] text-ink">
              <span data-h="line" className="block text-[13vw] leading-[0.95] sm:text-6xl lg:text-7xl xl:text-[5.2rem]">
                {profile.firstName}
              </span>
              <span data-h="line" className="text-google block text-[13vw] leading-[0.95] sm:text-6xl lg:text-7xl xl:text-[5.2rem]">
                {profile.lastName}
                <span className="text-ink">.</span>
              </span>
            </h1>

            <p data-h="role" className="mt-4 font-display text-[15px] font-bold leading-snug text-ink sm:text-lg">
              {profile.roleLineA} <span className="text-gray-300">·</span>{" "}
              <span className="text-google">{profile.roleLineB}</span>
            </p>

            <p data-h="tag" className="mt-3.5 max-w-xl text-[14.5px] leading-relaxed text-inksoft sm:text-base">
              {profile.tagline}
            </p>
            <p data-h="tag" className="mt-2.5 flex items-center gap-2 text-[12.5px] font-semibold italic text-gray-400 sm:text-[13px]">
              <Sparkles className="h-3.5 w-3.5 shrink-0 text-gyellow not-italic" />
              {profile.motto}
            </p>

            <div className="mt-7 flex flex-wrap items-center gap-3 sm:gap-3.5">
              <button
                type="button"
                data-h="cta"
                onClick={downloadResume}
                className="shine group flex flex-1 items-center justify-center gap-2.5 rounded-full bg-gblue px-6 py-3.5 text-sm font-bold text-white shadow-xl shadow-gblue/35 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-2xl hover:shadow-gblue/50 sm:flex-none"
              >
              ...
              <Link
                data-h="cta"
                to="/contact"
                className="glass group flex flex-1 items-center justify-center gap-2.5 rounded-full px-6 py-3.5 text-sm font-bold text-ink transition-all duration-300 hover:-translate-y-0.5 hover:border-gblue/40 sm:flex-none"
              >
                Contact Me
            ...
            <div data-h="stat" className="mt-10 grid grid-cols-2 gap-x-5 gap-y-7 border-t border-gray-200/70 pt-7 sm:mt-12 sm:grid-cols-4 sm:gap-x-6 sm:gap-y-8 sm:pt-8">
              {stats.map((s) => (
                <Stat key={s.label} {...s} />
              ))}
            </div>
